	...
	editStaffItem(item) {
	    ...
	    this.allStaff.forEach(staffItem => {
		    if(staffItem.key == item.key) {
				staffItem.info.name = item.info.name;
				staffItem.info.sex = item.info.sex;
				staffItem.info.age = item.info.age;
				staffItem.info.id = item.info.id;
				staffItem.info.descrip = item.info.descrip;
			}
		});
		this._sortStaff(this.sortType);
		this._filtStaff(this.filtType);
